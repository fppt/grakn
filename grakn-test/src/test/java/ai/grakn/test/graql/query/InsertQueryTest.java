...
public class InsertQueryTest extends AbstractMovieGraphTest {
    ...
    public void testKey() {
        ...
        assertTrue(qb.match(name("a-new-type").sub("entity").hasResource("a-new-resource-type")).ask().execute());
        assertTrue(qb.match(name("a-new-type").sub("entity").key("a-new-resource-type")).ask().execute());
        assertFalse(qb.match(name("a-new-type").sub("entity").key("title")).ask().execute());
        assertFalse(qb.match(name("movie").sub("entity").key("a-new-resource-type")).ask().execute());
